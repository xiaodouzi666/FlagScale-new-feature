...
class Wrapper:
        ...
        **kwargs,
        ...
        while True:
            ...
            try:
                ...
                if self._state.restart_attempt > 0:
                   try:
                       base_port = int(os.environ.get("MASTER_PORT_BASE", os.environ.get("MASTER_PORT", 29500)))
                       new_port = base_port + self._state.restart_attempt
                       os.environ["MASTER_PORT"] = str(new_port)
                       logger.info(f"Rank {self.rank}: Rotated MASTER_PORT to {new_port} for attempt {self._state.restart_attempt}")
                   except Exception as e:
    ...
    def trigger_restart(self, reason: str, error: Exception = None, fault_type: str = "manual") -> None:
