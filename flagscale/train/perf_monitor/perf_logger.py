...
class PerfMonitorLogger:
    ...
    def log_metrics(self, iteration: int, metrics_dict: Dict[str, float]):
        ...
        log_line += f"{samples_sec:<12.1f} {tokens_sec:<12.0f} {step_time:<10.1f} {memory_gb:<10.2f}"
        ...
        if len(log_files) > self.max_log_files:
            for old_file in log_files[:-self.max_log_files]:
                try:
                    ...
                    json_file = self.log_dir / f"perf_summary_{old_file.stem.replace('perf_metrics_', '')}.json"
                    if json_file.exists():
                        json_file.unlink()
