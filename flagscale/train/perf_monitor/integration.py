...
def setup_performance_monitor(args) -> FLOPSMeasurementCallback:
    ...
    log_interval = getattr(args, 'perf_log_interval',
                          getattr(args, 'log_interval', 100))
...
def add_performance_args(parser):
    ...
    group.add_argument('--perf-log-interval', type=int, default=100,
                      help='Interval for logging performance metrics')

    group.add_argument('--perf-memory-tracking', action='store_true',
                      help='Enable GPU memory usage tracking')

    group.add_argument('--perf-breakdown', action='store_true',
                      help='Log detailed FLOPS breakdown by component')
