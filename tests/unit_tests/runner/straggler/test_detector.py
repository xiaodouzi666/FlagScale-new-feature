...
class TestStragglerDetectorWithMockedDistributed:
    ...
    @patch('flagscale.runner.straggler.detector.torch', create=True)
    @patch('flagscale.runner.straggler.detector.dist', create=True)
    @patch('flagscale.runner.straggler.detector.TORCH_DISTRIBUTED_AVAILABLE', True)
    def test_gather_section_times_across_ranks(self, mock_dist, mock_torch, config):
        ...
        mock_torch.zeros.side_effect = lambda *args, **kwargs: FakeTensor()
        mock_torch.tensor.side_effect = lambda *args, **kwargs: FakeTensor()
