...
def example_3_standalone_flops_calculation():
    ...
    configurations = [
        {'name': 'GPT-3 (175B)', 'hidden_size': 12288, 'num_layers': 96,
         'num_attention_heads': 96, 'model_name': 'gpt'},
        {'name': 'LLaMA-7B', 'hidden_size': 4096, 'num_layers': 32,
         'num_attention_heads': 32, 'num_query_groups': 32, 'model_name': 'llama'},
        {'name': 'Mixtral-8x7B', 'hidden_size': 4096, 'num_layers': 32,
         'num_attention_heads': 32, 'num_experts': 8, 'moe_router_topk': 2,
         'model_name': 'mixtral'},
    ]
...
def example_5_detailed_breakdown():
    ...
    if breakdown:
        print(f"\nComponent Breakdown:")
        print(f"  Attention:       {breakdown['attention']/1e12:.2f} TFLOPS "
              f"({100*breakdown['attention']/breakdown['total']:.1f}%)")
        print(f"  FFN:             {breakdown['ffn']/1e12:.2f} TFLOPS "
              f"({100*breakdown['ffn']/breakdown['total']:.1f}%)")
        print(f"  Forward Pass:    {breakdown['forward']/1e12:.2f} TFLOPS "
              f"({100*breakdown['forward']/breakdown['total']:.1f}%)")
        print(f"  Backward Pass:   {breakdown['backward']/1e12:.2f} TFLOPS "
